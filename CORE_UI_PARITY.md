...
| 完整播放器 → 待播队列 | `Queue.kt`、`QueueMenu.kt`、`ShowMediaInfo.kt` | 完整播放器内查看、选择、改序、移除、清空待播；任意队列项可再次 Play next/Add to queue，歌曲可本地收藏并在登录后切换 YT Like、加入本地或可编辑云端歌单，Episode 可切换 Episodes for Later；另可管理离线下载、复制标准链接、从 YouTube Music 刷新真实元数据、打开本地与实时歌曲详情及所有带真实 ID 的 Artist/Album | 已实现，待真实账号桌面点击验收 |
