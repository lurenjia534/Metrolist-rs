...
- 阶段 1 内容过滤：Settings 对照 Android `ContentSettings.kt` 在 Appearance 提供 Hide explicit / Hide video songs / Hide YouTube Shorts 三个默认关闭开关。解析保留 explicit 徽章、`musicVideoType` 和去 `VL` 后以 `SS` 开头的歌单身份；Home、Explore、Search、Browse、Library 列表与由这些列表发起的 Play all 使用同一纯过滤函数。缺少真实标记的项保留，已在播放的队列不因开关改写，过滤后为空显示空态。SQLite v47 持久化三项开关及歌曲标记，非法设置不能半保存。
...
- 阶段 2 存储基线：建立桌面独立 SQLite v47 schema 和迁移检查，包含歌曲及播客单集身份、播放历史、识曲历史、搜索历史、队列项、单例播放会话及 Shuffle/Repeat 模式、当前播放缓存元数据、收藏、Song/Browse/Local Playlist Speed Dial、本地歌单及自定义封面、有序歌单项、播客收藏及取消墓碑、Episodes for Later、逐集播放位置、规范化歌词文档/行、单例应用设置、离线下载状态和参数均衡器档案；v1 至 v46 数据库会原位升级并保留已有数据，程序拒绝比当前版本更新的 schema。v18→v19 增加 Repeat/Shuffle 会话状态，v19→v20 增加识曲历史，v20→v21 增加搜索历史，v21→v22 增加本地 Album/Artist 目录，v22→v23 增加真实 song→album 映射，v23→v24 增加 Song Speed Dial，v24→v25 增加 Album/Artist/Playlist/Podcast Browse Speed Dial，v25→v26 增加随本地歌单删除级联的 Local Playlist Speed Dial，v26→v27 增加本地歌单自定义封面 URI，v27→v28 增加暂停本地收听与搜索历史设置，v28→v29 增加 1–100 秒播放历史时长，v29→v30 增加队列持久化设置，v30→v31 增加 Autoplay，v31→v32 增加跨队列保留 Shuffle 与跨重启记住 Shuffle/Repeat，v32→v33 增加 Repeat All 时停止自动补充，v33→v34 增加防止显式入队重复歌曲设置，v34→v35 增加播放错误自动跳过设置，v35→v36 增加优先随机原始播放列表/专辑设置，v36→v37 增加静音暂停设置，v37→v38 增加渐进封面 seek 设置，v38→v39 增加收藏后自动下载设置，v39→v40 增加自动 Radio 队列设置，v40→v41 增加 Radio 自动翻页设置，v41→v42 增加 Sleep Timer 曲末与淡出设置，v42→v43 增加 Skip silence 与 Instant skip 设置，v43→v44 增加 Crossfade、时长与 Gapless albums 设置，v44→v45 增加内容语言与国家/地区设置，v45→v46 增加默认关闭的七日 Automatic Sleep Timer schedule，v46→v47 增加 Hide explicit / Hide video songs / Hide YouTube Shorts 及歌曲 explicit/music-video-type 标记。数据库运行在专用线程，通过异步回复与 GPUI 交互，不读取或复用 Android Room 文件。
- 阶段 2 历史、统计与恢复：累计实际播放达到 Settings 保存的 1–100 秒门槛后写入本地历史，默认与 Android 一致为 30 秒。对照 Android `HistoryScreen`，侧栏提供独立 History 入口：Local 与已登录 YouTube Music 历史可切换并按标题/歌手过滤，本地记录可单删或确认清空；远端仅在项目携带非空 feedback token 时显示并执行精确删除，无 token 项不暴露静默无效入口；两者都复用播放、Next、Queue 和 Download。Library Overview 仍保留历史区，但不再是唯一入口。对照 Android `StatsScreen`，侧栏 Stats 按 7 天、30 天、3/6 个月、1 年或全部时间聚合 `play_history.play_time_ms`，展示总览、Top Songs、Top Artists 和 Top Albums；周期切换以请求 generation 保证只有最新选择可更新状态，旧查询不能覆盖当前 Tab；歌曲接入播放操作，真实歌手/专辑 ID 可进入详情。队列与当前播放状态定期保存；Persistent queue 开启时冷启动异步恢复且不会擅自播放，关闭时只恢复音量与 Repeat/Shuffle，不恢复歌曲、队列、位置或播放源。
...
- 阶段 2 日常设置：只读对照 Android 的 HTTP/SOCKS 代理及 Auto/Low/High 音质语义，新增强类型 `AppSettings`、SQLite 单例持久化（当前 schema v47）和完整设置页。应用会在任何首页网络请求前读取设置；代理（含遮罩认证字段）统一作用于 InnerTube、LRCLIB、缩略图、音频 Range 和 Last.fm 客户端，固定本地代理测试已证明请求实际经过代理而不只是保存配置。Low 选择不高于 128 kbps 的最佳直连 AAC，High 优先服务端高质量标记，Auto 选择最高码率；切换音质会清除当前安全源元数据、按新音质重新解析并保留进度和播放/暂停状态。内容语言与国家/地区、Hide explicit / Hide video songs / Hide YouTube Shorts、缓存根目录、音频容量、音量标准化级别、静音暂停、渐进封面 seek、Skip silence/Instant skip、Crossfade/时长/Gapless albums、Sleep Timer 曲末/淡出及七日自动日程、收藏后自动下载、自动 Radio 队列、Similar content、Auto load more、十段/参数均衡器、播放速度/移调模式、Repeat All 自动补充、队列持久化、Autoplay、播放错误自动跳过、跨队列 Shuffle、优先随机原始播放列表/专辑、跨重启 Shuffle/Repeat、防止显式入队重复歌曲、1–100 秒播放历史时长、本地收听/搜索历史暂停、YouTube Music 播放历史同步、Last.fm 行为、Discord Rich Presence 开关、“一起听”服务器/显示名/自动审批/房主音量同步和主题均可持久化；自定义根目录同时承载 `audio`/`thumbnails` 子缓存。保存会先验证代理 URL、内容 locale、绝对且非文件系统根的缓存路径、容量、Crossfade 时长、Automatic Sleep Timer 的 5–120 分钟时长与七日 00:00–23:59 窗口、Last.fm 阈值以及无凭据的 `ws://`/`wss://` 房间服务器，再在后台完整构造新服务并写库，任一步失败都保留旧服务；成功后才原子替换，旧音频线程也在后台退出，避免阻塞 GPUI；活动下载期间要求先暂停再应用网络或缓存设置，房间内也禁止切换服务器，避免两个服务实例并发操作同一持久目录或会话。
...
Radio 后端；Up next 从索引 0 显示当前项之前、当前项和未来项的完整队列，并与侧栏 Queue 共用
基于 video ID 的选择模式、全选/清空、数量和批量 Play、Shuffle、Add to queue、Add to local
playlist、Download、Remove selected；两处仍支持逐项改序/移除/清空。Lyrics 支持加载/重试/同步
行 seek，Related 提供相关推荐的
...
修改。Settings 另已接入默认关闭的 Automatic Sleep Timer：默认 30 分钟、每日 22:00–06:00，提供
Daily/Weekdays/Weekends 预设和逐日独立开关、HH:MM 开始/结束调整，跨午夜会延续到次日；播放仅在
非 Playing 转为 Playing、命中本地日程、没有活动 timer 且非 Guest 时复用现有 Deadline，继承曲末
停止与淡出，不因轮询或切歌重启。日程使用 SQLite v46 持久化，没有第二套播放器或计时服务。
...
侧栏与 Up next 另已补齐 Android `SelectionSongsMenu.kt` 的显式 Select 入口：选择按 video ID 共享，
支持逐项切换、Select all/Clear all、数量、Done，以及对当前选择按真实队列顺序 Play、Shuffle、
Add to queue、Add to local playlist、Download 和 Remove selected。批量移除包含当前项时先完成所有
倒序删除，再仅载入一次最终相邻项目；全部删空会停止播放并持久化空会话。退出相关覆盖层、替换队列
或完成批量动作会清空选择，Guest 不能进入选择模式或修改房主队列；没有新增平行队列或存储层。
...
for Later 及全部本地歌单关系中的真实歌曲，并搜索本地歌单。跨歌单按 video ID 去重，添加、移除或
删除歌单后统一重读；最后一个歌单关系消失且歌曲不属于其他来源时自然移出本地结果。All/Songs/
Playlists 过滤器、加载/错误/空状态均在同一页面可见；
...
保留；所有本地歌单歌曲也会恢复 SQLite v23 的真实 song→album 映射，因此其 Album 与 Artist 无需
预先在线浏览即可形成结果。点击复用现有 browse 详情，不根据标题猜测本地专辑。

主导航对照 Android 补齐 Stats：时间范围切换直接查询本地实际播放事件，总览与歌曲/歌手/专辑榜
均来自累计播放时长；每次有效周期切换会立即启动新 generation，旧周期查询的成功或失败不会覆盖
当前选择或清除最新任务状态。Top Songs 可整组或随机播放，并复用逐曲队列和下载动作。InnerTube renderer
...
及确认清空；远端继续使用服务端 feedback token 精确移除，但只有 token 存在且非空时才显示该动作，
缺少删除资格的远端项目不再提供静默无效入口。远端历史不复制到本地数据库。
...
目录与本地歌单；统一搜索非空时同时合并账号和设备已知歌曲，其中设备歌曲现包含全部本地歌单关系，
并从这些 Song 的真实 ID 派生 Overview 及本地 Albums/Artists。结果复用既有播放/详情链路，
...
歌曲与 Album/Artist/Playlist/Podcast/Profile 等目录，分别进入真实播放队列或 browse 详情；All
只有在两类结果均为空时才显示空态，总数和结果主体均覆盖两类数据，目录-only 首屏与 continuation
不会再被歌曲数量误判为空或无进展；其他筛选保持原请求参数、布局和可切换状态。
